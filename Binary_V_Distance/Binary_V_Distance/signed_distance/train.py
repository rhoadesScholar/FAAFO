...
from cellmap_data.transforms.targets import SignedDistanceTransform
...
def train(seed: int):
    ...
    print(f"Training signed distance with seed {seed}")
    ...
    loaders = get_dataloaders(
        batch_size, num_workers, spatial_transform, raw_transform=raw_transform, gt_transform=SignedDistanceTransform()
    )
    ...
    writer = SummaryWriter(f"logs/signed_distance_{seed}")
...
if __name__ == "__main__":
    print("Starting signed_distance training")
    if len(sys.argv) > 1:
        seed = int(sys.argv[1])
        train(seed)
    else:
        ...
        for seed in seeds:
            print(f"Launching signed_distance training for seed {seed}")
