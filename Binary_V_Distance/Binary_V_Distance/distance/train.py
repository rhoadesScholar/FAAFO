...
from cellmap_data.transforms.targets import DistanceTransform
...
def train(seed: int):
    ...
    print(f"Training distance with seed {seed}")
    ...
    loaders = get_dataloaders(
        batch_size, num_workers, spatial_transform, raw_transform=raw_transform, gt_transform=DistanceTransform()
    )
    ...
    writer = SummaryWriter(f"logs/distance_{seed}")
...
if __name__ == "__main__":
    print("Starting distance training")
    if len(sys.argv) > 1:
        seed = int(sys.argv[1])
        train(seed)
    else:
        ...
        for seed in seeds:
            print(f"Launching distance training for seed {seed}")
