...
from FourierSpaceSegmentation.calibration import get_student_type, STUDENT_TYPE_COLORS
import torch
...
for student_path in student_paths:
    ...
    student.eval()
    student.to(device)
    student_name = os.path.basename(student_path).split(".")[0].removeprefix("student_")
...
target_func = {
    ...
    "signed_distance": SignedDistanceTransform(),
}
post_func = {
    "BCE": torch.sigmoid,
    "fourier": lambda x: torch.abs(torch.fft.ifft2(x[:, 0] + 1j * x[:, 1])),
    "distance": lambda x: x,
    "signed_distance": lambda x: x,
}
# with thresholding
...
for i, (student_type, student) in enumerate(models.items()):
    ...
    ax[i, 2].imshow(
        output[0, 0].detach().cpu().numpy(), cmap="viridis", clim=tax.get_clim()
    )
    ax[i, 0].set_title(f"Input")
    ax[i, 1].set_title(f"Target: {student_type}")
    ax[i, 2].set_title(f"Output: {student_type}")
    plt.colorbar(
        ax[i, 1].imshow(target[0, 0].detach().cpu().numpy(), cmap="viridis"),
        ax=ax[i, 1],
    )
    plt.colorbar(
        ax[i, 2].imshow(output[0, 0].detach().cpu().numpy(), cmap="viridis"),
        ax=ax[i, 2],
    )
# %%
