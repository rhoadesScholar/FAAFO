...
from LocalOccupancyVectors.training_utils import (
    get_dataloaders,
    get_optimizer,
    get_scheduler,
    spatial_transform,
    ...
    n_epochs,
    steps,
    LocalOccupancyVectors,
)

kernel_size = (3, 3)

kernel_name = f"kernel{str(kernel_size).strip('()').replace(', ', 'x')}"

# %%
# Define the training function
def train(seed: int):

    print(f"Training Local Occupancy 3x3 Kernel with seed {seed}")
    ...
    from LocalOccupancyVectors.models import student
    student = student(np.prod(kernel_size))
    ...
    loaders = get_dataloaders(
        batch_size, num_workers, spatial_transform, raw_transform=raw_transform, gt_transform=LocalOccupancyVectors(kernel_size=kernel_size)
    )
    ...
    writer = SummaryWriter(f"logs/{kernel_name}_{seed}")
    ...
    save_path = os.path.join(
        ...
        "models",
        "checkpoints",
        f"Local Occupancy Vector_{kernel_name}_{seed}.pth",
    )
...
if __name__ == "__main__":
    print("Starting Local Occupancy Vector training")
    if len(sys.argv) > 1:
        seed = int(sys.argv[1])
        train(seed)
    else:
        ...
        for seed in seeds:
            print(f"Launching Local Occupancy Vector training with kernel size {kernel_size} for seed {seed}")
