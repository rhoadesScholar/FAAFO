...
from LocalOccupancyVectors.data.download import mitolab_prefix, cem_prefix

# Create Local Occupancy Vectors targets with defined kernel size
class LocalOccupancyVectors(torch.nn.Module):
    def __init__(self, kernel_size=(3, 3), in_channels=1, device=None):
        super().__init__()
        self.kernel_size = kernel_size
        self.ndims = len(kernel_size)
        ...
        if device is None:
            ...
            self.device = device
        conv = {1: torch.nn.Conv1d, 2: torch.nn.Conv2d, 3: torch.nn.Conv3d}[self.ndims]
        self.conv = conv(in_channels,np.prod(kernel_size),kernel_size, padding="same", bias=False)
        self.conv.weight.requires_grad = False
        self.conv.weight.data = torch.eye(np.prod(kernel_size)).reshape((np.prod(kernel_size),1,*kernel_size)).to(self.device)
